...
const LeaderboardBar: React.FC<{
  ...
  return (
    <Link
      href={`/houses/${houseStat.name.toLowerCase()}`}
      className={cn(
        "block p-3 sm:p-4 rounded-xl border border-border/60 bg-background/50 backdrop-blur-sm shadow-md transition-all duration-300 hover:shadow-lg hover:border-primary/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary",
    ...
    >
      <div className="flex items-center justify-between mb-2.5 gap-2">
        <div className="flex items-center space-x-2.5 sm:space-x-3 min-w-0">
          ...
          <div className="h-9 w-9 shrink-0 overflow-hidden rounded-full border border-primary/40 sm:h-11 sm:w-11">
            <Image
              src={house.crest}
              alt={`${house.name} 紋章`}
              width={44}
        ...
        <div className="text-right shrink-0">
          <p className="text-base sm:text-lg font-bold font-headline text-foreground">
            {houseStat.count.toLocaleString()}{" "}
...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    void fetch("/api/house-stats", { cache: "no-store" })
      ...
      .then((counts: Record<HouseName, number>) => {
        const stats: HouseStat[] = HOUSE_NAMES_ARRAY.map((name) => ({
          name,
          count: typeof counts[name] === "number" ? counts[name] : 0,
    ...
    <div className="py-4 sm:py-8 px-2 sm:px-4 max-w-xl sm:max-w-3xl mx-auto animate-fade-in-up">
      ...
      {isLoading ? (
        <div className="text-center text-primary py-12">
          リーダーボードの精霊を召喚中...{" "}
          <BarChart3 className="inline-block animate-spin h-5 w-5 ml-2" />
        </div>
